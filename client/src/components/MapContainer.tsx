		...
		setTimeout(() => {
			try {
				...
				if (bounds && bounds.getNorthEast() && bounds.getSouthWest()) {
					...
					setMapError(
						"Map failed to load. This is likely due to Google Maps API quota being exceeded. Please use a valid API key with billing enabled.",
					);
				}
			} catch {
				setMapError(
					"Map failed to initialize. Check your Google Maps API key configuration.",
				);
			}
		}, 1000);
	};
			...
			<div className="w-full h-full flex flex-col items-center justify-center bg-gray-50 p-8">
				<div className="max-w-md text-center">
					...
					<p className="text-sm text-gray-500">
						Check the browser console for details. The Google Maps API key may
						need to be configured in Render dashboard under{" "}
