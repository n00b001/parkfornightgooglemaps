import type { Handler } from 'https://deno.land/x/hono@v4.6.3/mod.ts'
 ...
 */
function transformPlace(place: Record<string, unknown>): Record<string, unknown> {
  const result = { ...place }
...
export const getStats: Handler = async (c) => {
  ...
  if (err1) {
    ...
    return c.json({ error: 'Failed to fetch stats', details: err1.message }, 500)
  }
  ...
  if (err2) {
    ...
    return c.json({ error: 'Failed to fetch stats', details: err2.message }, 500)
  }
  ...
  const { count: placesWithReviews, error: err3 } = await supabase
    ...
    .in(
      'id',
      (
        await supabase.from('review').select('placeId', { head: false }).limit(100000)
      ).data?.map((r) => r.placeId) ?? [],
    )
  if (err3) {
    console.error('Error fetching places with reviews:', err3.message)
    return c.json({ error: 'Failed to fetch stats', details: err3.message }, 500)
  }
