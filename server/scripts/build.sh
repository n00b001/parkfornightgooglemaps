...
npm install || {
	error "Server npm install failed."
	exit 1
}
...
if [ -z "${DATABASE_URL:-}" ]; then
	error "DATABASE_URL not set — cannot deploy migrations."
	exit 1
fi
npx prisma migrate deploy || {
	error "Prisma migrate deploy failed! Tables were NOT created."
	exit 1
}
