...
import { getPhotoThumbUrl, getVehicleIconUrl } from "../services/images";
...
const PlaceDetails: React.FC<any> = ({
	...
	const checkStreetViewAvailability = () => {
		...
		svService.getPanorama({ location, radius: 500 }, (data, status) => {
	...
	const addToGoogleMaps = () => {
		const googlePlaceId =
			googleDetails?.place_id ||
			place.google_place_id ||
			place.rawData?.google_place_id;
		...
		<div className="fixed bottom-4 left-4 right-4 md:w-[450px] z-40 bg-white rounded-3xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">
			...
			<div className="p-6 overflow-y-auto flex-1">
				<div className="flex justify-between items-start mb-1">
					<div>
						<span
							className={`inline-block text-[10px] font-bold px-2 py-0.5 rounded-full uppercase mb-2 ${TYPE_COLORS[place.type || ""] || "bg-gray-100 text-gray-600"}`}
						>
					...
					<div className="grid grid-cols-4 gap-3">
						{AMENITIES.map((amenity) => {
							...
							return (
								<div
									key={amenity.key}
									className="flex flex-col items-center p-3 bg-white rounded-2xl border border-gray-100 shadow-sm hover:border-blue-200 transition-colors"
								>
						...
						{!AMENITIES.some(
							(a) => place[a.key] === "1" || place.rawData?.[a.key] === "1",
						) && (
							<p className="text-sm text-gray-400 italic col-span-4">
							...
							<>
									...
									<div>
										...
										<div className="space-y-3">
											{reviews.map((r) => (
												<div
													key={r.id}
													className="text-sm bg-blue-50/30 p-3 rounded-xl border border-blue-100"
												>
								...
								{googleDetails?.reviews && googleDetails.reviews.length > 0 && (
									<div>
										...
										<div className="space-y-3">
											{googleDetails.reviews
												.slice(0, 3)
												.map((r: any, idx: number) => (
													<div
														key={idx}
														className="text-sm bg-gray-50 p-3 rounded-xl border border-gray-100"
													>
														<div className="flex justify-between mb-1">
