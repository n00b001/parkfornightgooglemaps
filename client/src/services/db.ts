    ...
    upgrade(db, oldVersion) {
      ...
      if (oldVersion < 4) {
        if (!db.objectStoreNames.contains(STORES.PENDING_FAVORITES)) {
          db.createObjectStore(STORES.PENDING_FAVORITES, { keyPath: 'placeId' });
        }
        if (!db.objectStoreNames.contains(STORES.PENDING_REVIEWS)) {
          db.createObjectStore(STORES.PENDING_REVIEWS, { keyPath: 'id', autoIncrement: true });
        }
      }
    },
  });
...
export const savePendingFavorite = async (placeId: number, action: 'add' | 'remove') => {
  const db = await initDB();
  await db.put(STORES.PENDING_FAVORITES, { placeId, action, timestamp: Date.now() });
};
...
export const savePendingReview = async (review: { placeId: number, content: string, rating: number }) => {
  const db = await initDB();
