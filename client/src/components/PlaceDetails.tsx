	...
	const fetchReviews = async () => {
		...
		try {
			const [localResult, p4nResult] = await Promise.all([
				supabase.functions.invoke("get-reviews", {
					body: { placeId: place.id },
				}),
				supabase.functions.invoke("get-place-reviews", {
					body: { placeId: place.id },
				}),
			]);
			const local = localResult.data ?? [];
			const p4n = p4nResult.data?.reviews ?? [];
			setReviews(local);
			setP4nReviews(p4n);
		...
		<div className="fixed bottom-4 left-4 right-4 md:w-[450px] z-40 bg-white rounded-3xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">
			...
			<div className="p-6 overflow-y-auto flex-1">
				<div className="flex justify-between items-start mb-1">
					<div>
						<span
							className={`inline-block text-[10px] font-bold px-2 py-0.5 rounded-full uppercase mb-2 ${TYPE_COLORS[place.type] || ""}`}
						>
				...
				<div className="mt-6">
					...
					<div className="grid grid-cols-4 gap-3">
						...
						{!AMENITIES.some((a) => place[a.key] === "1") && (
